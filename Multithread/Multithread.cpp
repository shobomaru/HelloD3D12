...
#define CB_ALIGNED_SIZE ((CB_SIZE + D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1) & ~(D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT - 1))
		...
		CHK(mDev->CreateCommittedResource(
			&CD3D12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
			D3D12_HEAP_MISC_NONE,
			&CD3D12_RESOURCE_DESC::Buffer(cbSize * MaxFrameLatency * MaxThreadCount),
			D3D12_RESOURCE_USAGE_GENERIC_READ,
			nullptr,
	...
	void Draw()
	{
		...
		setResourceBarrier(cmdListProl, mD3DBuffer.Get(), D3D12_RESOURCE_USAGE_PRESENT, D3D12_RESOURCE_USAGE_RENDER_TARGET);

		// Clear DepthTexture
		cmdListProl->ClearDepthStencilView(descHandleDsv, D3D12_CLEAR_DEPTH, 1.0f, 0, nullptr, 0);

		// Clear
		{
			float clearColor[4] = { 0.1f, 0.2f, 0.3f, 1.0f };
			cmdListProl->ClearRenderTargetView(descHandleRtv, clearColor, nullptr, 0);
		}
		...
		setResourceBarrier(cmdListEpir, mD3DBuffer.Get(), D3D12_RESOURCE_USAGE_RENDER_TARGET, D3D12_RESOURCE_USAGE_PRESENT);
